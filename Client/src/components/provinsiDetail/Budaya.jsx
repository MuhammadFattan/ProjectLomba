...
const Budaya = () => {
  ...
  useEffect(() => {
    if (!currentProvinsi?.budayaImg || currentProvinsi.budayaImg.length === 0) {
      return;
    }

    const interval = setInterval(() => {
      setCurr((prevCurr) =>
        prevCurr === currentProvinsi.budayaImg.length - 1 ? 0 : prevCurr + 1
      );
    }, 5000);

    return () => clearInterval(interval);
  }, [currentProvinsi?.budayaImg?.length]);

  const prev = () => {
    setCurr((prevCurr) =>
      prevCurr === 0 ? currentProvinsi.budayaImg.length - 1 : prevCurr - 1
    );
  };
  ...
  if (
    ...
    (!currentProvinsi?.budayaImg || currentProvinsi.budayaImg.length === 0)
  ) {
    return null;
  }
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-900 border-white text-white': 'bg-white border-gray-200'} rounded-lg border p-4 sm:p-6 mt-4 max-w-5xl mx-auto`}>
      <div className="flex flex-col md:flex-row gap-6">
        ...
        {/* Right Image Slider */}
        {currentProvinsi?.budayaImg?.length > 0 && (
