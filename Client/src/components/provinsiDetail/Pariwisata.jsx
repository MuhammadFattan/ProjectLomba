...
const pariwisata = () => {
  ...
  useEffect(() => {
    if (!currentProvinsi?.pariwisataImg || currentProvinsi.pariwisataImg.length === 0) {
      return;
    }

    const interval = setInterval(() => {
      setCurr((prevCurr) =>
        prevCurr === currentProvinsi.pariwisataImg.length - 1 ? 0 : prevCurr + 1
      );
    }, 5000);

    return () => clearInterval(interval);
  }, [currentProvinsi?.pariwisataImg?.length]);

  const prev = () => {
    setCurr((prevCurr) =>
      prevCurr === 0 ? currentProvinsi.pariwisataImg.length - 1 : prevCurr - 1
    );
  };
  ...
  if (
    !currentProvinsi?.pariwisataDesc &&
    (!currentProvinsi?.pariwisataImg || currentProvinsi.pariwisataImg.length === 0)
  ) {
    return null;
  }
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-900 border-white text-white': 'bg-white border-gray-200'} rounded-lg border p-4 sm:p-6 mt-4 max-w-5xl mx-auto`}>
      <div className="flex flex-col md:flex-row gap-6">
        ...
        {/* Right Image Slider */}
        {currentProvinsi?.pariwisataImg?.length > 0 && (
          <div className="flex-1 order-1 md:order-2">
            <div className="relative bg-gray-100 rounded-lg overflow-hidden">
              <div className="relative h-48 sm:h-64 lg:h-80">
                ...
                >
                  {currentProvinsi.pariwisataImg.map((item, index) => (
                    <div key={index} className="relative w-full h-full flex-shrink-0">
                      <img
                        src={item}
                        alt={`pariwisata ${index + 1}`}
